...
2.0.2 升级存在已知漏洞的 Python/系统依赖，并让镜像门禁阻断已有修复却未升级的高危漏洞，不再因上游暂无补丁的 CVE 永久卡住发布。2.0.1 修复 Docker 运行时权限门禁；现有 1.x 部署升级前仍须先备份 MySQL 与根目录 `.env`，再按照 [MIGRATING-2.0.md](MIGRATING-2.0.md) 完成 2.0 迁移。
...
- 扫描配置页新增数据库搜索关键词编辑器；保存后无需重启，下一轮扫描会自动将同一组关键词转换为 Quake、Hunter、DayDayMap 和 Fofa 各自的查询语法。
- 右上角执行提示按测速完成时间保持最新，切换历史等标签页后不会再回退到旧的“最近更新/耗时”。
- “立即测试”在 SSE 降级为短轮询的部署中会重新拉起进度同步，避免点击后测速状态卡住、日志不刷新，看起来像无法测速。
- 检测概览点击“日志”打开明细弹窗时，后端会优先读取本轮检测明细；若历史或异常轮次缺少明细落库，会按该轮时间窗口从持久化结果兜底返回频道检测状态，避免弹窗空白。
- 扫描结果“按来源分组”表格排序取消、质量筛选和手动检测后概览刷新体验已修复：取消排序不再导致表格消失，质量下拉会按真实数量过滤，手动检测完成后会主动刷新结果和检测概览。
...
- 主扫描搜索关键词存入数据库并可在扫描配置页动态修改；支持正文关键词、`title:` 标题关键词和 `&&` 多特征组合，下一轮扫描自动生效。
...
- C 段扫描：对已发现的 IP 所在 /24 子网进行扩展扫描，支持智能采样、单轮共享网段/IP 预算、单来源限额，以及按网段+端口缓存。
