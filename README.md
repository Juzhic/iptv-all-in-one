...
- Docker Compose 默认 MySQL 必须设置 `DB_PASSWORD`，推荐运行 `python generate_env.py` 自动生成随机密码。
- 默认 MySQL 使用独立容器 `mysql`，数据持久化在 Docker volume `mysql_data`，更新或重启应用容器不会清空数据库。
- 默认 MySQL 端口仅绑定到宿主机本地 `127.0.0.1:${MYSQL_HOST_PORT:-3306}`，方便本机访问且不暴露到局域网或公网。
