...
2.0.3 修复 Docker Hub 描述超过平台字节上限导致的发布尾部失败，并同步新版安全部署说明。2.0.2 升级存在已知漏洞的 Python/系统依赖并修正漏洞门禁；现有 1.x 部署升级前仍须先备份 MySQL 与根目录 `.env`，再按照 [MIGRATING-2.0.md](MIGRATING-2.0.md) 完成 2.0 迁移。
