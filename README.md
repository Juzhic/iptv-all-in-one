...
IPTV 频道测速、测绘采集与订阅管理工具，支持多 M3U 聚合、FFmpeg 质量检测、测绘平台采集、IP 探测、健康复检，以及桌面和移动端 Web 控制台。

当前版本：**2.1.2**。从 1.x 升级请先阅读 [MIGRATING-2.0.md](MIGRATING-2.0.md)，完整版本记录见 [CHANGELOG.md](CHANGELOG.md)。

## 主要功能

- 聚合多个 M3U 订阅，按模板和别名筛选频道。
- 对启用的数据源执行全量测速，检测分辨率、编码、带宽、延迟与稳定性，并生成 TXT/M3U。
- 通过 Quake、Hunter、DayDayMap、Fofa 等测绘平台采集 IPTV 候选源。
- 批量探测 IP、网段和端口，识别常见 IPTV 系统并生成候选源。
- 定期健康复检、移除失效频道、恢复可用频道并记录质量趋势。
