...
本项目当前以 SQLite 作为主要数据存储，默认数据库文件为 `database/data/iptv.db`。

旧版 README 中提到的 `config.json`、`subscribe.txt`、`alias.txt`、`demo.txt` 已不再作为日常配置入口。现在请通过 Web 后台的"系统配置"页面维护：

| 数据 | 当前存储位置 | 说明 |
| --- | --- | --- |
| 系统参数 | `database/data/iptv.db` 的 `config_data` 表，key 为 `config` | 测速时长、并发数、分辨率阈值、运行模式等 |
...
这些内容现在都在 Web 后台编辑，保存后写入 `database/data/iptv.db`。
...
**Q: 想清空历史或配置？**
历史数据在 `database/data/iptv.db` 中。删除数据库会同时清空配置、历史、日志和进度；删除前请先备份。
