...
│   ├── app.py                  # Flask 应用工厂、BasicAuth 中间件
│   ├── routes/                 # API 路由模块
...
│   ├── notifications.py        # 通知模块：测速完成、扫描结果等事件通知
...
│   ├── community_sources.py    # 社区源：从公开 IPTV 源获取频道
│   ├── isp_intelligence.py     # ISP 情报：运营商 IPTV 特征库和扫描策略
...
| `fofa_size` | `200` | Fofa 每次扫描返回的最大条数 |
| `enable_c_scan` | `true` | 是否开启 C 段扩展扫描 |
| `c_scan_limit` | `50` | C 段单段扫描 IP 数上限 |
| `c_segment_max_segments` | `8` | C 段最大子网数 |
| `c_segment_max_total_ips` | `200` | C 段最大总 IP 数 |
...
| `deep_batch_size` | `50` | 深度检测批次大小 |
| `scan_ports` | `[8080, 80, 443, 9981, 8888, 8000, 9090, 3000, 5000, 8443]` | DDGS 和 ISP 扫描的端口列表 |
| `ddgs_enabled` | `false` | 是否启用 DuckDuckGo 扫描 |
| `detection_interval_minutes` | `120` | 健康检测间隔（分钟） |
| `detection_cycle_timeout_minutes` | `30` | 检测周期超时（分钟） |
| `deletion_threshold` | `3` | 连续失败删除阈值 |
| `quality_history_keep_days` | `90` | 质量历史保留天数 |
| `stable_channel_multiplier` | `3` | 稳定频道倍数 |
| `resurrection_enabled` | `true` | 是否启用频道复活 |
| `resurrection_interval_hours` | `24` | 频道复活间隔（小时） |
| `isp_intelligence_enabled` | `false` | 是否启用 ISP 情报 |
| `hot_segment_min_channels` | `3` | 热门段最少频道数 |
| `hot_segment_scan_limit` | `200` | 热门段扫描限制 |
...
```json
{
  ...
  "data": {
    ...
    "checks": {
      ...
      "database": {"status": "ok", "size_mb": 15.3}
    }
  }
}
```
