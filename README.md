...
当前版本：**3.6.1**。3.0 起数据库已切换为 PostgreSQL，旧 MySQL 数据卷不能直接复用。完整升级步骤见 [MIGRATING-3.0.md](MIGRATING-3.0.md)，版本记录见 [CHANGELOG.md](CHANGELOG.md)。
...
3.6.0 将 UDPXY 并入原有扫描。在“配置中心 → 采集配置 → 高级采集策略”开启“UDPXY 组播识别”（默认关闭），保存后随下一轮手动/定时、全量/增量采集执行。

- 自动发现使用原有平台选择和 Key，支持 Quake、FOFA、Hunter、DayDayMap。主查询自动加入 UDPXY 标题规则，查询目标条数保持不变；开启质量优先查询时，UDPXY 与其他画像分配同一份画像预算，不再额外执行独立 Quake 搜索。
- 搜索结果与普通 IP 频道提取共用入口。发现 UDPXY 后匹配省份/运营商模板并进行流抽检，再将生成的 HTTP 候选交给原有逐条检测、候选池和复检流程；来源仍记为实际发现它的平台。
- 手动代理使用“IP 探测”：填写公网 IP:端口，选择“UDPXY（组播）”，指定省份和运营商后开始。无需测绘 API Key；留空沿用采集配置，无法确定省份时不会遍历全国模板。单选 UDPXY 可在自动识别关闭时使用，“全部”探测遵循自动识别开关。IP 探测结果仅为待测速候选，不将抽检结果视为整组质量合格。
- “UDPXY 频道模板”提供 36 组内置历史频道表及自定义 TXT/M3U。自定义内容如 `CCTV1,rtp://239.1.1.1:1234`；同省份/运营商的自定义模板替换内置模板，每组最多 128000 字符、2000 个目的地址，最多 32 组、合计 512000 字符。

高级策略中的“UDPXY 展开限制”默认每轮最多探测 20 个代理与模板组合、展开 500 条频道；所有平台共享这些上限，各可用代理轮流分配频道预算。最多 5 个并发探测，验证阶段限时 120 秒，超时保留已验证候选，停止请求会终止后续展开。状态页可访问不代表可播放，需抽检有效 MPEG-TS，最终质量仍以逐条测速为准。

3.6.1 补齐 UDPXY 的 C 段拓展：采集阶段以通过 TS 抽检的代理为起点，探测同 C 段、同端口的邻近 IP，保留 `/udpxy` 等代理前缀；与普通源共用 C 段总预算、单来源预算和近期扫描缓存，且计入组播代理/频道上限。只拓展一轮，不从新发现的邻居递归扩散；生成的候选仍需逐条测速。手动 IP 探测只处理填写的目标，不自动扩大范围。

开启“复检后自动拓展”后，本轮质量达标的 UDPXY 源也会用于补源。它与普通源共用复检拓展的 IP/网段/深测上限、任务租约和持久化冷却（默认 24 小时），并受组播代理/频道上限约束。能唯一匹配配置模板时展开模板；因历史记录信息缺失或频道表变化而无法唯一匹配时，只沿用该条已验证频道地址。每个邻居先抽检 TS，再逐条深测，质量不达标的频道不会入池。关闭组播识别或 C 段探测即停用对应 UDPXY 拓展；超时、取消仍保留已写入的冷却记录。

升级保留 3.5.0 的开关、模板、代理/频道上限及手动代理。旧版独立 Quake 开关和搜索预算不再使用；搜索由平台选择及现有主查询、画像预算统一控制。已保存代理仍随采集使用，也可在展开限制中清空后改用 IP 探测。UDPXY 代理仅接受公网 IP，拒绝账号、查询参数、播放路径及跳转；旧版保存的 HTTP(S) 基础地址与 `/udpxy` 前缀继续有效。

内置模板来自 spider-iptv 的 2024 年历史快照，不保证当前有效。程序仅提取组播目的地址，忽略广告、旧代理主机、HTML 错误页和重复项，不会自动下载外部模板。源数据及 Apache-2.0 许可证随镜像包含，详见[模板说明](scanner_integration/data/spider_iptv/NOTICE.md)。
...
| 页面空白或静态资源 404 | 镜像标签是否为 3.6.1，源码部署是否完成前端构建 |
