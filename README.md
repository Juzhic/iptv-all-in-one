...
├── scanner_integration/        # 扫描模块（整合自 itv_scan）
...
├── templates/
│   └── index.html              # Web 后台页面（7个标签页）
├── static/
│   ├── js/
│   │   ├── index.js            # 测速模块前端逻辑
│   │   └── scan.js             # 扫描模块前端逻辑（含定时扫描倒计时）
│   └── css/
│       └── scan.css            # 扫描模块样式（可选）
...
| 标签 | 说明 |
| --- | --- |
| 总览 | 通过率趋势、统计卡片、亮点和关注频道 |
| 历史记录 | 测速历史列表，可展开查看详情 |
| 按频道 | 按频道名查看所有测速结果 |
| 系统测速 | 触发/停止测速、实时进度、下载订阅地址 |
| 系统配置 | 编辑订阅源、频道模板、别名映射和系统参数 |
| 频道扫描 | 配置扫描参数、API Key、定时扫描、触发扫描、查看进度 |
| 扫描结果 | 查看扫描频道列表、按分类/省份过滤、送入测速 |
