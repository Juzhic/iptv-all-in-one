# -*- coding: utf-8 -*-
"""
scanner_integration 包入口�?提供异步扫描到同�?Flask 路由的桥接层�?"""
import asyncio
import threading
import time
import uuid
...
def _scan_log(msg):
    ...
    try:
        ...
        pass  # DB 写入失败不影响扫描流�?    logger.info(msg)
    ...
    def start(self):
        with self._start_lock:
            ...
            if self._loop is not None and self._thread is not None and self._thread.is_alive():
                # 线程已启动但循环还没标记 running，等一�?                for _ in range(50):
...
def init_bridge():
    ...
    # 初始化多 Key 管理�?    try:
        ...
        logger.warning(f"[Bridge] KeyManager 初始化失�? {e}")
    # 启动定时扫描任务
    start_daily_task()
    # 启动每日数据库维护任�?    start_daily_db_maintenance()
    # 启动定期检测任�?    try:
        from .detection import detection_manager
        detection_manager.start()
    except Exception as e:
        logger.warning(f"[Bridge] 检测模块启动失�? {e}")


# ==================== 扫描编排协程 ====================

async def _do_scan(platforms_override=None, provinces_override=None):
    """执行完整的扫描流程：采集 �?快速过�?�?深度检�?�?保存数据库�?""
    ...
    # 覆盖配置（仅在内存中修改，不回写数据库，避免覆盖用户 API Key�?    if platforms_override is not None:
    ...
    try:
        # 阶段1：采�?        _scan_log(f"[Scan:{scan_id}] 开始采�?..")
        ...
        if not raw:
            _scan_log(f"[Scan:{scan_id}] 采集完成，未获取到任何频道。请检�?API Key 是否已配置�?)
            _db.update_scan_run(scan_id, status='completed', finished_at=_db.now_str(),
                                total_raw=0)
            _db.update_scan_progress(running=False, phase='idle', message='采集完成，未获取到频�?,
                                     percent=100)
        ...
        _scan_log(f"[Scan:{scan_id}] 采集�?{len(raw)} 条，去重�?{len(uniq)} �?)
        _db.update_scan_run(scan_id, total_raw=len(raw), total_deduped=len(uniq))

        # ISP Intelligence：分析历史数据中的热点段并主动扫�?        if cfg.get('isp_intelligence_enabled'):
            try:
                from .isp_intelligence import scan_hot_segments
                _scan_log(f"[Scan:{scan_id}] ISP Intelligence: 正在分析热点�?..")
                ...
                if isp_channels:
                    ...
                    for ch in uniq:
                        ...
                        if not ch.get('city'):
                            ch['city'] = ''
                    _db.update_scan_run(scan_id, total_raw=len(raw), total_deduped=len(uniq))
                    _scan_log(f"[Scan:{scan_id}] ISP Intelligence 合并完成，去重后 {len(uniq)} �?)
                else:
        ...
        if cfg.get('community_sources_enabled'):
            try:
                ...
                if cs_channels:
                    _scan_log(f"[Scan:{scan_id}] 社区�? 发现 {len(cs_channels)} 个频道，正在合并...")
                    ...
                    for ch in uniq:
                        ...
                        if not ch.get('city'):
                            ch['city'] = ''
                    _db.update_scan_run(scan_id, total_raw=len(raw), total_deduped=len(uniq))
                    _scan_log(f"[Scan:{scan_id}] 社区源合并完成，去重�?{len(uniq)} �?)
                else:
                    _scan_log(f"[Scan:{scan_id}] 社区�? 未发现新频道")
            except Exception as e:
                _scan_log(f"[Scan:{scan_id}] 社区源扫描异�? {e}")
                logger.warning(f"[Scan:{scan_id}] 社区源扫描异�? {e}")

        _db.update_scan_progress(phase='fast_filter', total=len(uniq),
                                 message=f'快速过�?{len(uniq)} 个频�?..')

        # 阶段2：快速过�?        _scan_log(f"[Scan:{scan_id}] 开始快速过�?{len(uniq)} 个频�?..")
        ...
        # 批量写入数据�?        _db.insert_scan_results(scan_id, quick)
        _scan_log(f"[Scan:{scan_id}] 快速过滤完成，{len(quick)} 个频道存�?)

        # 更新扫描状�?        scan_state.channels = quick
        scan_state.failure_counts = {c['url']: 0 for c in quick}
        scan_state.last_update_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        scan_state.initialized = True

        # 阶段3：后台深度检�?        _db.update_scan_progress(phase='deep_check', total=len(quick), processed=0,
                                 percent=0, message=f'深度检�?{len(quick)} 个频�?..')

        _scan_log(f"[Scan:{scan_id}] 启动后台深度检�?..")
        await background_deep_update(quick, log_fn=_scan_log)

        # �?video_check 模块级状态读取深度检测结�?        from . import video_check as _vc
        ...
        _db.update_scan_progress(running=False, phase='idle', percent=100,
                                 message=f'扫描完成，{len(quick)} 个频�?)
        _scan_log(f"[Scan:{scan_id}] 扫描完成，耗时 {duration:.1f}s，共 {len(quick)} 个频�?)
        ...
        # 合并到持久化结果�?        try:
            ...
            _scan_log(f"[Scan:{scan_id}] 合并到持久化结果集失�? {type(e).__name__}: {e}")
            logger.warning(f"[Scan:{scan_id}] 合并到持久化结果集失�? {type(e).__name__}: {e}")
...
def start_daily_task():
    ...
    logger.info("[Scheduler] 定时扫描任务已启�?)
    ...
    while True:
        try:
            ...
            for day_offset in range(8):
                ...
                # weekday(): 0=Mon, 6=Sun �?�?update_days 编号一�?                if candidate.weekday() in update_days and candidate > now:
                    target = candidate
                    break

            if target is None:
                logger.warning("[Scheduler] 未找到匹配的扫描时间�?0 秒后重试")
                await asyncio.sleep(60)
                continue

            wait_seconds = (target - now).total_seconds()
            logger.info(f"[Scheduler] 下次扫描: {target.strftime('%Y-%m-%d %H:%M')} "
                        f"(等待 {wait_seconds:.0f} �?")
            await asyncio.sleep(wait_seconds)

            # 到达目标时间，检查是否有正在进行的扫�?            import database as _db
    ...
    while True:
        try:
            ...
            deleted = _db.cleanup_old_run_logs(days=RUN_LOGS_RETENTION_DAYS)
                logger.info(f"[DBMaint] 清理过期 run_logs: {deleted} �?)
            except Exception as e:
            ...
            try:
                ...
                if progress.get('running'):
                    logger.info("[DBMaint] 扫描正在进行中，跳过 VACUUM 以避免并发写入风�?)
                else:
            ...
            # 防止同一天重复触�?            await asyncio.sleep(60)

        except asyncio.CancelledError:
            logger.info("[DBMaint] 维护任务被取�?)
            break
        except Exception as e:
